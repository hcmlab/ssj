...
public class StrategyLoaderTest
{
	...
	public void testEventNames()
	{
		...
		for (Feedback feedback : feedbackList.get(0).keySet())
		{
			if(feedback instanceof AuditoryFeedback)
			{
				assertTrue(Arrays.equals(
						feedback.getOptions().eventNames.get(),
						new String[]{ StrategyLoader.THRESHOLD_PREFIX+"0", StrategyLoader.THRESHOLD_PREFIX+"1"}
				));
			}
			else if(feedback instanceof AndroidTactileFeedback)
			{
				assertTrue(Arrays.equals(
						feedback.getOptions().eventNames.get(),
						new String[]{ StrategyLoader.THRESHOLD_PREFIX+"0"}
				));
			}
			else if(feedback instanceof VisualFeedback)
			{
				assertTrue(Arrays.equals(
						feedback.getOptions().eventNames.get(),
						new String[]{ StrategyLoader.THRESHOLD_PREFIX+"0"}
		...
		for (Feedback feedback : feedbackList.get(1).keySet())
		{
			if(feedback instanceof AuditoryFeedback)
			{
				assertTrue(Arrays.equals(
						feedback.getOptions().eventNames.get(),
						new String[]{ StrategyLoader.THRESHOLD_PREFIX+"2", StrategyLoader.THRESHOLD_PREFIX+"3"}
				));
			}
			else if(feedback instanceof AndroidTactileFeedback)
			{
				assertTrue(Arrays.equals(
						feedback.getOptions().eventNames.get(),
						new String[]{ StrategyLoader.THRESHOLD_PREFIX+"1",StrategyLoader.THRESHOLD_PREFIX+"2"}
				));
			}
			else if(feedback instanceof VisualFeedback)
			{
		...
		for (Feedback feedback : feedbackList.get(2).keySet())
		{
			if(feedback instanceof AuditoryFeedback)
			{
				assertTrue(Arrays.equals(
						feedback.getOptions().eventNames.get(),
						new String[]{ StrategyLoader.THRESHOLD_PREFIX+"4"}
				));
			}
			else if(feedback instanceof AndroidTactileFeedback)
			{
				assertTrue(Arrays.equals(
						feedback.getOptions().eventNames.get(),
						new String[]{ StrategyLoader.THRESHOLD_PREFIX+"3",StrategyLoader.THRESHOLD_PREFIX+"4"}
				));
			}
			else if(feedback instanceof VisualFeedback)
			{
