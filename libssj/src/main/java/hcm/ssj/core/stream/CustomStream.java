...
public class CustomStream extends Stream
{
    ...
    public CustomStream(int num, int dim, int bytes, double sr)
    {
        this.num = num;
        this.dim = dim;
        this.bytes = bytes;
        this.type = Cons.Type.CUSTOM;
        this.sr = sr;
        this.step = 1.0 / sr;
    ...
    public byte[] ptr()
{
    ...
    public CustomStream select(int[] new_dims)
    {
        if(dim == new_dims.length)
            return this;

        CustomStream slice = new CustomStream(num, new_dims.length, bytes, sr);
        slice.source = source;
    ...
    public CustomStream select(int new_dim)
    {
        if(dim == 1)
            return this;

        CustomStream slice = new CustomStream(num, 1, bytes, sr);
        slice.source = source;
    ...
    public Stream clone()
    {
        CustomStream copy = new CustomStream(num, dim, bytes, sr);
